...
\vskip 50pt
\begin{center}
{\bf \LARGE NLP Ridge and Lasso logistic regression}
...
The aim of this paper is to apply Ridge, Lasso and elastic net binary logistic regression to a natural laguage procesing (NLP) problem. Additionally, some clasic text preprocesing techniques will be introduced.  

The calculations and visuals are made using the statistical software $\textbf{R}$. The main framework for text preprocesing is the $\textbf{text2vec}$ `r Cite(bib, 'text2vec')` package. The code snipets are included in the article for reproducibility. 
...
The target collumn obtains two values: 0 if the question is sincere and 1 if the question is not sincere. 
...
Only about 6 percent of the documents in the corpus have a negative sentiment. We will balance the data by taking the same number of sincere sentiment rows as the insinsere ones.
...
We will hold out some data when creating a model in order to see if our model will generalize well. Ideally, the proportion of two clases in the test dataset must be similar to the overall dataset.

We will hold out 30 percent of the original dataset.
...
We will create seceral models with diferent alpha values using the glmnet package `r Cite(bib, 'glm', .opts = list(cite.style = "numeric"))`. The first model will represent the Lasso logistic regression while the last one will represent the Ridge logistic regression.
